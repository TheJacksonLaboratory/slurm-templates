...
#SBATCH --array=1-4%1        # runs array, 1 at a time.  
...
LIB=$(sed -n -e "${SLURM_ARRAY_TASK_ID} p" /fastscratch/${USER}/adir/slurm-templates-main/workshop_examples/list_lib_names.txt)

echo "${LIB}_R1.fq    ${LIB}_R2.fq" > array_run1_${SLURM_ARRAY_TASK_ID}_out_${LIB}_file_names.txt
